...
public struct Gemma4PatchedTextConfiguration: Codable, Sendable {
...
class Gemma4PatchedRMSNormNoScale: Module {
    let eps: Float
...
class Gemma4PatchedScaledLinear: Module {
    let weight: MLXArray
    let scalar: Float
...
enum Gemma4PatchedPositionOffset {
    case scalar(Int)
    case batch(MLXArray)
}

private func gemma4PatchedCapturePositionOffset(from cache: KVCache?) -> Gemma4PatchedPositionOffset {
...
private func gemma4PatchedApplyRotaryPosition<R: RoPELayer>(
    _ rope: R,
    to x: MLXArray,
    offset: Gemma4PatchedPositionOffset
) -> MLXArray {
    switch offset {
...
class Gemma4PatchedAttention: Module {
    let config: Gemma4PatchedTextConfiguration
    let layerIdx: Int
    ...
    @ModuleInfo(key: "v_norm") var vNorm: Gemma4PatchedRMSNormNoScale

    @ModuleInfo var rope: RoPELayer

    init(_ config: Gemma4PatchedTextConfiguration, layerIdx: Int) {
        self.config = config
        ...
        self._vNorm.wrappedValue = Gemma4PatchedRMSNormNoScale(eps: config.rmsNormEps)
    ...
    func callAsFunction(
        ...
        positionOffset: Gemma4PatchedPositionOffset? = nil
    ) -> (MLXArray, (MLXArray, MLXArray), Gemma4PatchedPositionOffset) {
        ...
        let keys: MLXArray
        let values: MLXArray
        let activePositionOffset = positionOffset ?? gemma4PatchedCapturePositionOffset(from: cache)
        ...
        queries = gemma4PatchedApplyRotaryPosition(rope, to: queries, offset: activePositionOffset)
...
class Gemma4PatchedDecoderLayer: Module {
    let config: Gemma4PatchedTextConfiguration
    let layerIdx: Int
    let layerType: String
    let hiddenSizePerLayerInput: Int

    @ModuleInfo(key: "self_attn") var selfAttn: Gemma4PatchedAttention
    @ModuleInfo var mlp: Gemma4PatchedMLP
    ...
    init(_ config: Gemma4PatchedTextConfiguration, layerIdx: Int) {
        ...
        self._selfAttn.wrappedValue = Gemma4PatchedAttention(config, layerIdx: layerIdx)
        self._mlp.wrappedValue = Gemma4PatchedMLP(config, layerIdx: layerIdx)
...
class Gemma4PatchedTextModelInner: Module {
    let config: Gemma4PatchedTextConfiguration
    let embedScale: Float
    let hiddenSizePerLayerInput: Int

    @ModuleInfo(key: "embed_tokens") var embedTokens: Embedding
    @ModuleInfo(key: "layers") var layers: [Gemma4PatchedDecoderLayer]
    @ModuleInfo var norm: RMSNorm

    // Per-layer embeddings (PLE)
    @ModuleInfo(key: "embed_tokens_per_layer") var embedTokensPerLayer: Embedding?
    @ModuleInfo(key: "per_layer_model_projection") var perLayerModelProjection: Gemma4PatchedScaledLinear?
    ...
    init(_ config: Gemma4PatchedTextConfiguration) {
        ...
        if config.hiddenSizePerLayerInput > 0 {
            ...
            self._perLayerModelProjection.wrappedValue = Gemma4PatchedScaledLinear(
    ...
    func callAsFunction(
        ...
        var intermediates = [(kv: (MLXArray, MLXArray)?, positionOffset: Gemma4PatchedPositionOffset?)](
...
public class Gemma4PatchedTextModel: Module, LLMModel, KVCacheDimensionProvider {
    public let vocabularySize: Int
    public let kvHeads: [Int]

    fileprivate let config: Gemma4PatchedTextConfiguration
    let model: Gemma4PatchedTextModelInner
...
extension Gemma4PatchedTextModel: LoRAModel {
...
/// This is a thin wrapper around `Gemma4PatchedTextConfiguration` that handles the
/// nested `text_config` structure from HuggingFace model configs.
public struct Gemma4PatchedConfiguration: Codable, Sendable {
    var modelType: String = "gemma4"
    var textConfig: Gemma4PatchedTextConfiguration
    var vocabSize: Int = 262144
    ...
    public init(from decoder: Decoder) throws {
        ...
        if let textConfig = try container.decodeIfPresent(
            Gemma4PatchedTextConfiguration.self, forKey: .textConfig)
        {
            self.textConfig = textConfig
            // Propagate vocab_size into text config
            self.textConfig.vocabSize = self.vocabSize
        } else {
            self.textConfig = try Gemma4PatchedTextConfiguration(from: decoder)
        }
    }
}
...
public class Gemma4PatchedModel: Module, LLMModel, KVCacheDimensionProvider {
    public var vocabularySize: Int { languageModel.vocabularySize }
    public var kvHeads: [Int] { languageModel.kvHeads }

    @ModuleInfo(key: "language_model") fileprivate var languageModel: Gemma4PatchedTextModel

    public init(_ config: Gemma4PatchedConfiguration) {
        self._languageModel.wrappedValue = Gemma4PatchedTextModel(config.textConfig)
    }
...
extension Gemma4PatchedModel: LoRAModel {
    public var loraLayers: [Module] {
